...
function createArtworksListReader(
  ...
  const action = searchArtworks(query, pagination).then(
    (payload) => payload.data
  );

  return getSuspenseReader(action);
}
      ...
      <div className={cn(styles.contentContainer)}>
        <div className={cn(styles.content)}>
          <div className={cn(styles.avatar)}>
            <SmartImage artworkReader={artworkReader} />
          </div>
